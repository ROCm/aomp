module array_m
      ...
      type r1_t
         double precision, allocatable :: val(:)       !# <VAR NAME=val TYPE=real(:)>Values</VAR>
      ...
      subroutine print_r1(r1)
         ...
         do while (.not.done)
            ...
            if (ar1%dim1 .lt. 1) then
               ...
               write(6,'(a,f14.2,3(a,i0,a,f14.2))')"From val[1]=",ar1%val(1),", val[",2,"]=",ar1%val(2),",&
               val[",ar1%dim1-1,"]=",ar1%val(ar1%dim1-1),", val[",ar1%dim1,"]=",ar1%val(ar1%dim1)
            endif
...
!$omp target data map(to:fin,PB,PC) map(from:PA)
!$omp target teams  distribute parallel do
         do i=1, fin
            !A%val(i)=0.5*(B%val(i)+C%val(i))
            PA(i)=0.5*(PB(i)+PC(i))
         enddo
