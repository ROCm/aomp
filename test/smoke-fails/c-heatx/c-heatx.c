...
while ( error >= epsilon && istep <= nstep ) {
        ...
        error = 0.0 ;
#pragma omp target teams distribute parallel for thread_limit(1024) reduction(max:error) collapse(3) schedule(static,1)
for( k = k1; k <= kn; k++) {
for( j = j1; j <= jn; j++) {
for( i = i1; i <= in; i++) {
...
#pragma omp target teams distribute parallel for thread_limit(1024) collapse(3) schedule(static,1)
for( k = k1; k <= kn; k++) {
for( j = j1; j <= jn; j++) {
for( i = i1; i <= in; i++) {
        a(i,j,k) = anew(i,j,k) ;
