...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd schedule(static,sch)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd schedule(dynamic, sch)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd dist_schedule(static,sch)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	{
#pragma omp distribute parallel for simd dist_schedule(static) schedule(static)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
	}
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd dist_schedule(static) schedule(static,sch)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd dist_schedule(static,sch) schedule(static)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd dist_schedule(static,sch) schedule(dynamic)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	    {
#pragma omp distribute parallel for simd dist_schedule(static,dssch) schedule(dynamic,sch)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	    }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd dist_schedule(static,sch) schedule(guided)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	    {
#pragma omp distribute parallel for simd dist_schedule(static,dssch) schedule(guided,sch)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	    }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd dist_schedule(static,sch) schedule(auto)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
...
#pragma omp teams num_teams(tms) thread_limit(ths)
	  {
#pragma omp distribute parallel for simd dist_schedule(static,sch) schedule(runtime)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	  }
