...
int main(void) {
  ...
  //
  #undef TARGET_TDPARALLEL_FOR_CLAUSES
  #define TARGET_TDPARALLEL_FOR_CLAUSES proc_bind(master)
  #include "ttdpf_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
    int num_teams = cpuExec? 1 : tms;
    TARGET_TDPARALLEL_FOR1(
    {
      S[0] = 0;
      for (int i = 0; i < N; i++) {
        A[i] = B[i] = 0;
  ...
  #undef TARGET_TDPARALLEL_FOR_CLAUSES
  #define TARGET_TDPARALLEL_FOR_CLAUSES proc_bind(close)
  #include "ttdpf_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
  ...
  #undef TARGET_TDPARALLEL_FOR_CLAUSES
  #define TARGET_TDPARALLEL_FOR_CLAUSES proc_bind(spread)
  #include "ttdpf_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
  ...
  //
  #undef TARGET_TDPARALLEL_FOR_CLAUSES
  #define TARGET_TDPARALLEL_FOR_CLAUSES private(p,q) shared(A,B,C,D,E)
  #include "ttdpf_defines.h"
  ...
  //
  #undef TARGET_TDPARALLEL_FOR_CLAUSES
  #define TARGET_TDPARALLEL_FOR_CLAUSES firstprivate(p,q)
  #include "ttdpf_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
  ...
  //
  #undef TARGET_TDPARALLEL_FOR_CLAUSES
  #define TARGET_TDPARALLEL_FOR_CLAUSES lastprivate(q)
  #include "ttdpf_defines.h"
  // FIXME: modify to t=1 and in ttdpf_defines.h to use host after bug fix.
  ...
  //
  #undef TARGET_TDPARALLEL_FOR_CLAUSES
  #define TARGET_TDPARALLEL_FOR_CLAUSES collapse(2)
  #include "ttdpf_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	t++;
#pragma omp target teams distribute parallel for num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      } // loop over 'ths'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
    ...
    for (int tms = 1 ; tms <= 256 ; tms *= 2) { // 8 times
      for (int ths = 32 ; ths <= 1024 ; ths *= 2) { // 6 times
	t++;
#pragma omp target teams distribute parallel for schedule(static) num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      } // loop over 'ths'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target teams distribute parallel for schedule(static,sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
    ...
    for (int tms = 1 ; tms <= 256 ; tms *= 2) { // 8 times
      for (int ths = 32 ; ths <= 1024 ; ths *= 2) { // 6 times
	  t++;
#pragma omp target teams distribute parallel for schedule(dynamic) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
      } // loop 'ths'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1 ; sch <= n ; sch *= 1200) {
	  t++;
#pragma omp target teams distribute parallel for schedule(dynamic, sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
    ...
    for (int tms = 1 ; tms <= 256 ; tms *= 2) { // 8 times
      for (int ths = 32 ; ths <= 1024 ; ths *= 2) { // 6 times
	t++;
#pragma omp target teams distribute parallel for dist_schedule(static) num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      }
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 128 ; sch <= n ; sch *= 10000) {
	  t++;
#pragma omp target teams distribute parallel for dist_schedule(static,sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
    ...
    for (int tms = 1 ; tms <= 256 ; tms *= 2) { // 8 times
      for (int ths = 32 ; ths <= 1024 ; ths *= 2) { // 6 times
	t++;
#pragma omp target teams distribute parallel for dist_schedule(static) schedule(static) num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      }
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1 ; sch <= n ; sch *= 1000) { // speed up very slow tests
	  t++;
#pragma omp target teams distribute parallel for dist_schedule(static) schedule(static,sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 128 ; sch <= n ; sch *= 1200) {
	  t++;
#pragma omp target teams distribute parallel for dist_schedule(static,sch) schedule(static) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int dssch = 128 ; dssch <= n ; dssch *= 1200) {
	  for(int sch = 100 ; sch <= n ; sch *= 3000) {
	    t++;
#pragma omp target teams distribute parallel for dist_schedule(static,dssch) schedule(static,sch) num_teams(tms) thread_limit(ths)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	  } // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 128 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target teams distribute parallel for dist_schedule(static,sch) schedule(dynamic) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int dssch = 128 ; dssch <= n ; dssch *= 3000) {
	  for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	    t++;
#pragma omp target teams distribute parallel for dist_schedule(static,dssch) schedule(dynamic,sch) num_teams(tms) thread_limit(ths)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	  } // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target teams distribute parallel for dist_schedule(static,sch) schedule(guided) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int dssch = 1000 ; dssch <= n ; dssch *= 3000) {
	  for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	    t++;
#pragma omp target teams distribute parallel for dist_schedule(static,dssch) schedule(guided,sch) num_teams(tms) thread_limit(ths)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	  } // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target teams distribute parallel for dist_schedule(static,sch) schedule(auto) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target teams distribute parallel for dist_schedule(static,sch) schedule(runtime) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
