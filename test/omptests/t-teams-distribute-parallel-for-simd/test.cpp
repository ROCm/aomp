...
int main(int argc, char *argv[]) {
  ...
  //
  #undef TDPARALLEL_FOR_SIMD_CLAUSES
  #define TDPARALLEL_FOR_SIMD_CLAUSES proc_bind(master)
  #include "tdpfs_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
    int num_teams = cpuExec? 1 : tms;
    TDPARALLEL_FOR_SIMD1(
    {
      S[0] = 0;
      for (int i = 0; i < N; i++) {
        A[i] = B[i] = 0;
  ...
  #undef TDPARALLEL_FOR_SIMD_CLAUSES
  #define TDPARALLEL_FOR_SIMD_CLAUSES proc_bind(close)
  #include "tdpfs_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
  ...
  #undef TDPARALLEL_FOR_SIMD_CLAUSES
  #define TDPARALLEL_FOR_SIMD_CLAUSES proc_bind(spread)
  #include "tdpfs_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
  ...
  //
  #undef TDPARALLEL_FOR_SIMD_CLAUSES
  #define TDPARALLEL_FOR_SIMD_CLAUSES private(p,q) shared(A,B,C,D,E)
  #include "tdpfs_defines.h"
  ...
  //
  #undef TDPARALLEL_FOR_SIMD_CLAUSES
  #define TDPARALLEL_FOR_SIMD_CLAUSES firstprivate(p,q)
  #include "tdpfs_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
  ...
  //
  #undef TDPARALLEL_FOR_SIMD_CLAUSES
  #define TDPARALLEL_FOR_SIMD_CLAUSES lastprivate(q)
  #include "tdpfs_defines.h"
  // FIXME: modify to t=1 and in tdpfs_defines.h to use host after bug fix.
  ...
  //
  #undef TDPARALLEL_FOR_SIMD_CLAUSES
  #define TDPARALLEL_FOR_SIMD_CLAUSES collapse(2)
  #include "tdpfs_defines.h"
  for (int tms = 1; tms <= max_teams; tms *= 3) {
  for (int t = 1; t <= max_threads; t+=78) {
    int threads[1]; threads[0] = t;
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	t++;
#pragma omp target
#pragma omp teams distribute parallel for simd num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      } // loop over 'ths'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	t++;
#pragma omp target
#pragma omp teams distribute parallel for simd schedule(static) num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      } // loop over 'ths'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd schedule(static,sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
    ...
    for (int tms = 1 ; tms <= 256 ; tms *= 2) { // 8 times
	  ...
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd schedule(dynamic) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
      } // loop 'ths'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1 ; sch <= n ; sch *= 1200) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd schedule(dynamic, sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static) num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      }
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 128 ; sch <= n ; sch *= 10000) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static) schedule(static) num_teams(tms) thread_limit(ths)
	  for (int i = 0; i < n; ++i) {
	    a[i] += b[i] + c[i];
	  }
      }
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1 ; sch <= n ; sch *= 1000) { // speed up very slow tests
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static) schedule(static,sch) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 128 ; sch <= n ; sch *= 1200) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,sch) schedule(static) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int dssch = 128 ; dssch <= n ; dssch *= 1200) {
	  for(int sch = 100 ; sch <= n ; sch *= 3000) {
	    t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,dssch) schedule(static,sch) num_teams(tms) thread_limit(ths)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	  } // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 128 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,sch) schedule(dynamic) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int dssch = 128 ; dssch <= n ; dssch *= 3000) {
	  for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	    t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,dssch) schedule(dynamic,sch) num_teams(tms) thread_limit(ths)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	  } // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,sch) schedule(guided) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int dssch = 1000 ; dssch <= n ; dssch *= 3000) {
	  for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	    t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,dssch) schedule(guided,sch) num_teams(tms) thread_limit(ths)
	      for (int i = 0; i < n; ++i) {
		a[i] += b[i] + c[i];
	      }
	  } // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,sch) schedule(auto) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
...
#pragma omp target update to(a[:n],b[:n],c[:n])
	...
	for(int sch = 1000 ; sch <= n ; sch *= 3000) {
	  t++;
#pragma omp target
#pragma omp teams distribute parallel for simd dist_schedule(static,sch) schedule(runtime) num_teams(tms) thread_limit(ths)
	    for (int i = 0; i < n; ++i) {
	      a[i] += b[i] + c[i];
	    }
	} // loop 'sch'
