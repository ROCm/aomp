...
!                                                                         !
!                       O p e n A C C     V E R S I O N                   !
!                                                                         !
!                                   F T                                   !
!                                                                         !
!-------------------------------------------------------------------------!
!                                                                         !
!    This benchmark is an OpenACC version of the NPB FT code.             !
...
!$omp target update to(u)
      call fft(1, u1, u0)
!!$omp target update from(u1, u0)
!      print*, "GJDEBUG:", sum(u1), sum(u0)
...
!         call fft(-1, u1, u2)
         call fft(-1, u1, u1)
!!$omp target update from(u1)
!      print*, "GJDEBUG:", sum(u1)
!         call checksum(iter, u2, dims(1), dims(2), dims(3))
#ifndef FAIL
         call checksum_fail(iter, u1, dims(1), dims(2), dims(3))
#else
         call checksum_pass(iter, u1, dims(1), dims(2), dims(3))
#endif
      end do
...
!---------------------------------------------------------------------
!!$omp target teams distribute parallel do map(to:starts) thread_limit(1)
      do k = 1, dims(3) 
         x0 = starts(k)
         do j = 1, dims(2) 
...
!---------------------------------------------------------------------
 ...
 1000 format(//,' NAS Parallel Benchmarks (NPB3.4-ACC)',  &
     &          ' - FT Benchmark', /)
...
!$omp target update to(fftblock,fftblockpad)
...
!      do k = 1, maxdim*maxdim*maxdim
!                y1(k)=0
!                y2(k)=0
!      enddo
!      double complex y1(maxdim*maxdim*maxdim),  &
!     &               y2(maxdim*maxdim*maxdim)
...
!---------------------------------------------------------------------
      ...
      if (dir .eq. 1) then
         call cffts1p( dims(1), dims(2), dims(3), x1, x1, y1, y2)
         call cffts2p( dims(1), dims(2), dims(3), x1, x1, y1, y2)
         call cffts3p( dims(1), dims(2), dims(3), x1, x2, y1, y2)
      else
         call cffts3n( dims(1), dims(2), dims(3), x1, x1, y1, y2)
         call cffts2n( dims(1), dims(2), dims(3), x1, x1, y1, y2)
         call cffts1n( dims(1), dims(2), dims(3), x1, x2, y1, y2)
      endif
      return
      end
...
!---------------------------------------------------------------------
      ...
      subroutine cffts1p( d1, d2, d3, x, xout, y1, y2)
...
!---------------------------------------------------------------------
      ...
      integer  d1, d2, d3, logd1,l
      double complex x(d1+1,d2,d3)
      double complex xout(d1+1,d2,d3)
      double complex y1(maxdim,maxdim,maxdim), y2(maxdim,maxdim,maxdim)
      integer i, j, k, jn,jj, iz,kz,jz
      integer n1,li,lj,lk,ku,i11,i12,i21,i22
...
!$omp target teams distribute parallel do  &
!$omp& private(x11,x21,i11,i12,i21,i22,u1)
      do k = 1, d3
        do j=1, d2
               do i = 1, d1
                  y1(i,j,k) = x(i,j,k)
      ...
      do iz = 0, li - 1
        ...
        do kz = 0, lk - 1
            x11 = y1(i11+kz,j,k)
            x21 = y1(i12+kz,j,k)
            y2(i21+kz,j,k) = x11 + x21
            y2(i22+kz,j,k) = u1 * (x11 - x21)
        enddo
      enddo
      ...
      do iz = 0, li - 1
        ...
        do kz = 0, lk - 1
            x11 = y2(i11+kz,j,k)
            x21 = y2(i12+kz,j,k)
            y1(i21+kz,j,k) = x11 + x21
            y1(i22+kz,j,k) = u1 * (x11 - x21)
        enddo
      enddo
      endif
      enddo
...
!---------------------------------------------------------------------
      ...
      subroutine cffts1n( d1, d2, d3, x, xout, y1, y2)
...
!---------------------------------------------------------------------
      ...
      subroutine cffts2p( d1, d2, d3, x, xout, y1, y2)
...
!---------------------------------------------------------------------
      ...
      integer is, d1, d2, d3, logd2, l
      double complex x(d1+1,d2,d3)
      double complex xout(d1+1,d2,d3)
      double complex y1(maxdim,maxdim,maxdim), y2(maxdim,maxdim,maxdim)
      integer i, j, k, ii, iz,kz,jz,in
      integer n1,li,lj,lk,ku,i11,i12,i21,i22
      double complex u1,x11,x21
...
!$omp& private(x11,x21,i11,i12,i21,i22,u1)
      ...
      do iz = 0, li - 1
        ...
        do kz = 0, lk - 1
            x11 = y1(i11+kz,i,k)
            x21 = y1(i12+kz,i,k)
            y2(i21+kz,i,k) = x11 + x21
            y2(i22+kz,i,k) = u1 * (x11 - x21)
        enddo
      enddo
      ...
      do iz = 0, li - 1
        ...
        do kz = 0, lk - 1
            x11 = y2(i11+kz,i,k)
            x21 = y2(i12+kz,i,k)
            y1(i21+kz,i,k) = x11 + x21
            y1(i22+kz,i,k) = u1 * (x11 - x21)
        enddo
      enddo
      endif
      enddo
...
!---------------------------------------------------------------------
      ...
      subroutine cffts2n( d1, d2, d3, x, xout, y1, y2)
...
!---------------------------------------------------------------------
      ...
      subroutine cffts3p(d1, d2, d3, x, xout, y1, y2)
...
!---------------------------------------------------------------------
      ...
      integer d1, d2, d3, logd3, l
      double complex x(d1+1,d2,d3)
      double complex xout(d1+1,d2,d3)
      double complex y1(maxdim,maxdim,maxdim), y2(maxdim,maxdim,maxdim)
      integer i, j, k, j1,iz,kz,jz
      integer n1,li,lj,lk,ku,i11,i12,i21,i22      
      double complex u1,x11,x21
      logd3 = ilog2(d3)
      n1 = d3 / 2
...
!$omp& private(x11,x21,i11,i12,i21,i22,u1)
      ...
      do iz = 0, li - 1
        ...
        do kz = 0, lk - 1
            x11 = y1(i,i11+kz,j)
            x21 = y1(i,i12+kz,j)
            y2(i,i21+kz,j) = x11 + x21
            y2(i,i22+kz,j) = u1 * (x11 - x21)
        enddo
      enddo
      ...
      do iz = 0, li - 1
        ...
        do kz = 0, lk - 1
            x11 = y2(i,i11+kz,j)
            x21 = y2(i,i12+kz,j)
            y1(i,i21+kz,j) = x11 + x21
            y1(i,i22+kz,j) = u1 * (x11 - x21)
        enddo
      enddo
     endif
      enddo
...
!---------------------------------------------------------------------
      ...
      subroutine cffts3n( d1, d2, d3, x, xout, y1, y2)
...
!---------------------------------------------------------------------
      ...
      double complex y1(maxdim,maxdim,maxdim), y2(maxdim,maxdim,maxdim)
      integer i, j, k, j1,iz,kz,jz
      integer m,n1,li,lj,lk,ku,i11,i12,i21,i22      
      double complex u1,x11,x21
      logd3 = ilog2(d3)
      n1 = d3 / 2
!$omp target teams distribute parallel do  collapse(2) &
...
!---------------------------------------------------------------------
      ...
      use ft_data, only: fftblock, fftblockpad, u
      implicit none
!      external fftz2
!!$omp declare target to(fftz2) 
...
!---------------------------------------------------------------------
      do l = 1, m, 2
        call fftz2 (is, l, m, n, fftblock,  u, x, y,0,0)
        if (l .eq. m) goto 160
        call fftz2 (is, l + 1, m, n, fftblock, u, y, x,0,0)
      enddo
...
!---------------------------------------------------------------------
      ...
      subroutine fftz2 (is, l, m, n, ny, u, x, y,n0,k0)
...
!!$omp declare target
      ...
      integer n0,k0,iz,kz,jz
      double complex u,x,y,u1,x11,x21
      dimension u(n), x(n0,n0,n0), y(n0,n0,n0)
!!$omp declare target (u,x,y)
!---------------------------------------------------------------------
...
!---------------------------------------------------------------------
        do kz = 0, lk - 1
          do jz = 1, ny
            x11 = x(k0, jz,i11+kz)
            x21 = x(k0, jz,i12+kz)
            y(k0,jz,i21+kz) = x11 + x21
            y(k0,jz,i22+kz) = u1 * (x11 - x21)
          enddo
        enddo
      enddo
...
subroutine checksum_pass(i, u1, d1, d2, d3)
...
!---------------------------------------------------------------------
      ...
      double complex chk
      chk = (0.0,0.0)
!$omp target teams distribute parallel do private(q,r,s) &
!$omp& reduction(+:chk)
      do j=1,1024
         q = mod(j, nx)+1
         r = mod(3*j,ny)+1
...
!---------------------------------------------------------------------
      ...
      subroutine checksum_fail(i, u1, d1, d2, d3)
...
!$omp& reduction(+:cr,ci) 
      do j=1,1024
         ...
         r = mod(3*j,ny)+1
         s = mod(5*j,nz)+1
         cr = cr +REAL(u1(q,r,s),8)
         ci = ci +DIMAG(u1(q,r,s))
      end do
      chk=CMPLX(cr,ci,8)
      chk = chk/ntotal_f
