#!/bin/bash
#
# Checks all tests in usm directory using make check. Programs return 0 for success or a number > 0 for failure.
...
sed -i '/pfspecifier/ {s/0/Check the run.log above/}; /devices/ {s/0/Check the run.log above/}; /stream/ {s/0/Check the run.log above/}' check-usm.txt
echo ""
if [ -e check-usm.txt ]; then
  cat check-usm.txt
fi
if [ -e make-fail.txt ]; then
  cat make-fail.txt
