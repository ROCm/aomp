...
void OMP45Stream<T>::copy()
{
  ...
  T *a = this->a;
  T *c = this->c;
  #pragma omp target teams loop map(to: a[0:array_size], c[0:array_size])
...
void OMP45Stream<T>::mul()
{
  ...
  T *b = this->b;
  T *c = this->c;
  #pragma omp target teams loop map(to: b[0:array_size], c[0:array_size])
  for (int i = 0; i < array_size; i++)
  {
    b[i] = scalar * c[i];
  }
...
void OMP45Stream<T>::add()
{
  ...
  T *b = this->b;
  T *c = this->c;
  #pragma omp target teams loop map(to: a[0:array_size], b[0:array_size], c[0:array_size])
  for (int i = 0; i < array_size; i++)
  {
    c[i] = a[i] + b[i];
  }
...
T OMP45Stream<T>::dot()
{
  ...
  T *a = this->a;
  T *b = this->b;
  #pragma omp target teams loop reduction(+:sum) map(tofrom: sum)
  for (int i = 0; i < array_size; i++)
  {
    sum += a[i] * b[i];
  }
