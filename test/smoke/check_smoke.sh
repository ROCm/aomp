...
function printfails(){
  ...
  if [ "${EPSDB}" == 1 ] ; then
    ...
    if [ -e "${file}" ]; then
      ...
      # shellcheck disable=SC2094  # ${file} is only read here, never written
      while read -r line; do
        line=${line/: Make Failed/}
        echo
...
if [ "$1" == "-clean" ]; then
  for directory in ${SMOKE_DIRS}; do
    ...
    if ! pushd "${directory}" > /dev/null; then continue; fi
    make clean
    popd > /dev/null || exit
  done
  exit 0
...

{
  ...
  echo "Tests that need to be visually inspected: devices, stream"
  echo "***********************************************************************************"
} > check-smoke.txt

known_fails=""
skip_tests=""
# shellcheck disable=SC2010  # version-sorted listing of /opt; glob would not preserve ls ordering
  ...
  if sem --help > /dev/null; then
    ...
    for directory in ${SMOKE_DIRS}; do
      ...
      if [ "${base}" == 'hip_rocblas' ] ; then
        if ! ls "${AOMPROCM}/rocblas" > /dev/null 2>&1; then
