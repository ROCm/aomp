...
module mpas_ut_func
   ...
   subroutine kernel1(nVertLevels, maxEdges, nCells, nEdges, nCellsSolve, cellStart, cellEnd, vertexStart, vertexEnd, edgeStart, edgeEnd, &
                                   ...
                                   dts, small_step, epssm, cf1, cf2, cf3, T_e2e&
                                   )
      use m_random
      implicit none
...
!$acc end data
      ...
      write(1, '(F21.18)') wwAvg
      write(1, '(F21.18)') rho_pp
      write(1, '(F21.18)') rtheta_pp
...
program test
    ...
    real (kind=RKIND), parameter :: dts = 0.5
    ...
    do i=1,iter
        call kernel1(40,6,14760,44640,14040,1,14760,1,29880,1,44640,1,14040,1,28080,1,42120,dts,small_step,epssm,cf1,cf2,cf3, t_iter)
        t_sum = t_sum + t_iter
    end do
