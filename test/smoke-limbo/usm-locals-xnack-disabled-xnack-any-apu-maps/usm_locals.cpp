...
// usm_locals: XNACK disabled, compiled with: gfx90a, OMPX_APU_MAPS enabled, no USM pragma => OUTCOME: COPY
...
int main() {
  ...
  /// CHECK: __tgt_rtl_data_submit_async: {{.*}} 0 ({{.*}} 20, {{.*}})
  ...
  /// CHECK: __tgt_rtl_data_retrieve_async: {{.*}} 0 ({{.*}} 20, {{.*}})
  ...
  // enabled.
  /// CHECK-NOT: Device pointer for k = [[K_HOST_ADDR]], k[3] = -3
