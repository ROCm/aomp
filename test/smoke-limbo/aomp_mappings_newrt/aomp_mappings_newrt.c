...
int main()
{
  ...
  int remainder = 0;
  int errors = 0;
  int actual_num_threads_in_parallel = -1;
...
#pragma omp target teams distribute parallel for thread_limit(4) map(from:actual_num_threads_in_parallel)
  {
    for (int j = 0; j< N; j++) {
       ...
       if (j == 0) actual_num_threads_in_parallel = omp_get_num_threads();
    }
  }
  ...
  for (i = 0; i < N; i++){
    ...
    //check thread #
    if (thread_num[i] != i % actual_num_threads_in_parallel)
    ...
    //check lane #
    if (lane_id[i] != i % actual_num_threads_in_parallel)
    ...
    //check num threads
    if (num_threads[i] > actual_num_threads_in_parallel )
      recordError(&errors, "NUM THREADS", i, num_threads, NULL);

    //check num teams
    //If number of iterations is not divisible by actual_num_threads_in_parallel get the ceiling
    if(N % THREAD_LIMIT != 0)
    ...
    //check active mask
    mask = 0;
    if(N % actual_num_threads_in_parallel != 0){
      remainder = N % actual_num_threads_in_parallel;
    ...
    if (i < N - remainder){
      // Expected mask depends on the number of threads actually
      // executing the parallel region
      for (int j = 0 ; j < actual_num_threads_in_parallel; j++){
        mask = mask << 1;
        mask = mask + 1;
      }
    }
...
#pragma omp target teams distribute parallel for
  ...
  for (i = 0; i < N; i++){
    ...
    //check device #, We use default device (-1) for testing
    if (default_dev[i] != -1)
      recordError(&errors, "DEVICE NUMBER", i, default_dev, NULL);
