// TODO: Remove the start/stop/flush calls from the user
// program. Every time an init-device callback is received, a new
// device handle is obtained. Start the trace using this handle. Add
// this device to a vector of devices. During ompt_finalize, traverse
// the vector, flushing and stopping all the devices one by one.
#include <cassert>
#include <map>
...
typedef std::map<int, ompt_device_t*> DeviceMap_t;
...
static int start_trace(ompt_device_t *Device) {
...
(
  ...
  assert(DeviceMap.find(device_num) == DeviceMap.end() &&
	 "Device already present in the map");
  DeviceMap.insert(std::make_pair(device_num, device));
