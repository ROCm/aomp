...
int main(int argc, char* argv[]) {
  ...
  double* c_m_gate;
  hipMalloc(&c_m_gate, sizeof(double)*nCells);
  hipMemcpy(c_m_gate, m_gate, sizeof(double)*nCells, hipMemcpyHostToDevice);
  double* c_Vm;
  hipMalloc(&c_Vm, sizeof(double)*nCells);
  hipMemcpy(c_Vm, Vm, sizeof(double)*nCells, hipMemcpyHostToDevice);
