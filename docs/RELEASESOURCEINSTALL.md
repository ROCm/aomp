...
```
   wget https://github.com/ROCm-Developer-Tools/aomp/releases/download/rel_16.0-3/aomp-16.0-3.tar.gz
   tar -xzf aomp-16.0-3.tar.gz
   cd aomp16.0
   nohup make &
```
...
The build_aomp.sh script will install into $HOME/rocm/aomp_16.0-3 and create a symbolic link from $HOME/rocm/aomp to $HOME/rocm/aomp_16.0-3.
This feature allows multiple versions of AOMP to be installed concurrently.
To enable a backlevel version of AOMP, simply set AOMP to $HOME/rocm/aomp_16.0-2.
...
```
   wget https://github.com/ROCm-Developer-Tools/aomp/blob/aomp-16.0-3/bin/package.py
   spack create -n aomp -t makefile --force https://github.com/ROCm-Developer-Tools/aomp/releases/download/rel_16.0-3/aomp-16.0-3.tar.gz
   spack edit aomp
   spack install aomp
```
