...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then 
  ...
  if [ "$AOMP_BUILD_PERF" == "1" ]; then
    ...
    if [ "$AOMP_BUILD_SANITIZER" == 1 ]; then
       ...
       echo rm -rf $BUILD_DIR/build/openmp_perf_asan
       rm -rf $BUILD_DIR/build/openmp_perf_asan
       mkdir -p $BUILD_DIR/build/openmp_perf_asan
       cd $BUILD_DIR/build/openmp_perf_asan
