...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then 
   ...
   if [ "$AOMP_LEGACY_OPENMP" == "1" ]; then
      ...
      if [ "$AOMP_BUILD_SANITIZER" == 1 ]; then
        ASAN_CMAKE_OPTS="$COMMON_CMAKE_OPTS -DCMAKE_PREFIX_PATH=$ROCM_CMAKECONFIG_PATH;$AOMP_INSTALL_DIR/lib/asan/cmake $AOMP_ASAN_ORIGIN_RPATH -DSANITIZER_AMDGPU=1 -DCMAKE_BUILD_TYPE=Release"
