...
cfgvar() {
  get_config_var_string rocsparse "$1"
}

cfgbool() {
  get_config_var_bool rocsparse "$1"
}

_repo_dir="$(cfgvar AOMP_REPOS)/rocmlibs/rocSPARSE"
...
get_build_dir() {
   ...
   case "$Cfg" in
   "default")
     echo -n "$(cfgvar BUILD_DIR)/rocmlibs/rocSPARSE"
     ;;
   *)
...
task_cmake() {
   ...
   echo " -----Running cmake for rocSPARSE $Cfg ---- "
...
task_build() {
   ...
   pushd "$BuildDir" >& /dev/null || exit
   echo " -----Running make for rocSPARSE $Cfg ---- "
   if ! make -j"$Jobs"; then
