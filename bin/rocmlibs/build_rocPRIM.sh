...
cfgvar() {
  get_config_var_string rocprim "$1"
}

cfgbool() {
  get_config_var_bool rocprim "$1"
}

_repo_dir="$(cfgvar AOMP_REPOS)/rocmlibs/rocPRIM"
...
get_build_dir() {
   ...
   case "$Cfg" in
   "default")
     echo -n "$(cfgvar BUILD_DIR)/rocmlibs/rocPRIM"
     ;;
   *)
...
task_cmake() {
   ...
   echo " -----Running cmake for rocPRIM $Cfg ---- "
...
task_build() {
   ...
   pushd "$BuildDir" >& /dev/null || exit
   echo " -----Running $AompCmake --build for rocPRIM $Cfg ---- "
   echo "$AompCmake --build . -j $Jobs"
   if ! env CXX="$AOMP_INSTALL_DIR/bin/hipcc" "$AompCmake" --build . -j "$Jobs"; then
