...
cfgvar() {
  get_config_var_string rocsolver "$1"
}

cfgbool() {
  get_config_var_bool rocsolver "$1"
}

_repo_dir="$(cfgvar AOMP_REPOS)/rocmlibs/rocSOLVER"
...
get_build_dir() {
   ...
   case "$Cfg" in
   "default")
     echo -n "$(cfgvar BUILD_DIR)/rocmlibs/rocSOLVER"
     ;;
   *)
...
task_cmake() {
   ...
   echo " -----Running cmake for rocSOLVER $Cfg ---- "
...
task_build() {
   ...
   pushd "$BuildDir" >& /dev/null || exit
   echo " -----Running make for rocSOLVER $Cfg ---- "
   if ! make -j"$Jobs"; then
