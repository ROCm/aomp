...
cfgvar() {
  get_config_var_string hipblas "$1"
}

cfgbool() {
  get_config_var_bool hipblas "$1"
}

_repo_dir="$(cfgvar AOMP_REPOS)/rocmlibs/hipBLAS"
...
get_build_dir() {
   ...
   case "$Cfg" in
   "default")
     echo -n "$(cfgvar BUILD_DIR)/rocmlibs/hipBLAS"
     ;;
   *)
...
task_cmake() {
   ...
   echo " -----Running cmake for hipBLAS $Cfg ---- "
...
task_build() {
   ...
   pushd "$BuildDir" >& /dev/null || exit
   echo " -----Running make for hipBLAS $Cfg ---- "
   if ! make -j"$Jobs"; then
