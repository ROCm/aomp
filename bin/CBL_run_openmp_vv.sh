...
export MY_OPENMP_VV_FLAGS="-g -foffload=-march=gfx90a -m64 -fopenmp -O2"
...
if [ "$make_target" == "all" ] ; then
  ...
  make CC=$CBLC CXX=$CBLCXX FC=$CBLF90 CFLAGS="-lm $MY_OPENMP_VV_FLAGS" CXXFLAGS="$MY_OPENMP_VV_FLAGS" FFLAGS="$EFFLAGS $MY_OPENMP_VV_FLAGS"  OMP_VERSION=4.5 LOG=1 LOG_ALL=1 VERBOSE_TESTS=1 VERBOSE=1 all
  echo
else
  echo
  pwd
...
#  fi
  ...
  echo " make CC=$CBLC CXX=$CBLCXX FC=$CBLF90 CFLAGS="-lm $MY_OPENMP_VV_FLAGS" CXXFLAGS="$MY_OPENMP_VV_FLAGS" FFLAGS="$EFFLAGS $MY_OPENMP_VV_FLAGS" LOG=1 LOG_ALL=1 VERBOSE_TESTS=1 VERBOSE=1 OMP_VERSION=$this_omp_version SOURCES=$single_case all"
make CC=$CBLC CXX=$CBLCXX FC=$CBLF90 CFLAGS="-lm $MY_OPENMP_VV_FLAGS" CXXFLAGS="$MY_OPENMP_VV_FLAGS" FFLAGS="$EFFLAGS $MY_OPENMP_VV_FLAGS" LOG=1 LOG_ALL=1 VERBOSE_TESTS=1 VERBOSE=1 OMP_VERSION=$this_omp_version SOURCES=$single_case all
  rc=$?
  echo
  ...
  if [ -f $AOMP_REPOS_TEST/$AOMP_OPENMP_VV_REPO_NAME/bin/${single_case}.o ] ; then
     ...
     echo "       Expected binary $AOMP_REPOS_TEST/$AOMP_OPENMP_VV_REPO_NAME/bin/${single_case}.o does NOT exists!"
  fi
  echo
  popd
  exit $rc
...
#export MY_OPENMP_VV_FLAGS="$MY_OPENMP_VV_FLAGS -fopenmp-version=50"
make tidy
make CC=$CBLC CXX=$CBLCXX FC=$CBLF90 CFLAGS="-lm $MY_OPENMP_VV_FLAGS" CXXFLAGS="$MY_OPENMP_VV_FLAGS" FFLAGS="$EFFLAGS $MY_OPENMP_VV_FLAGS"  OMP_VERSION=5.0 LOG=1 LOG_ALL=1 VERBOSE_TESTS=1 VERBOSE=1 all
echo 
...
if [ "$ROCMASTER" != "1" ] && [ "$EPSDB" != "1" ] && [ "$SKIP_OPENMP_VV51" != 1 ]; then
echo "--------------------------- START OMP 5.1 TESTING ---------------------"
# Run OpenMP 5.1 Tests
#export MY_OPENMP_VV_FLAGS="$MY_OPENMP_VV_FLAGS -fopenmp-version=51"
make tidy
make CC=$CBLC CXX=$CBLCXX FC=$CBLF90 CFLAGS="-lm $MY_OPENMP_VV_FLAGS" CXXFLAGS="$MY_OPENMP_VV_FLAGS" FFLAGS="$EFFLAGS $MY_OPENMP_VV_FLAGS"  OMP_VERSION=5.1 LOG=1 LOG_ALL=1 VERBOSE_TESTS=1 VERBOSE=1 all
echo 
...
#export MY_OPENMP_VV_FLAGS="$MY_OPENMP_VV_FLAGS -fopenmp-version=51"
make tidy
make CC=$CBLC CXX=$CBLCXX FC=$CBLF90 CFLAGS="-lm $MY_OPENMP_VV_FLAGS" CXXFLAGS="$MY_OPENMP_VV_FLAGS" FFLAGS="$EFFLAGS $MY_OPENMP_VV_FLAGS"  OMP_VERSION=5.2 LOG=1 LOG_ALL=1 VERBOSE_TESTS=1 VERBOSE=1 all
echo 
