...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then 
   ...
   MYCMAKEOPTS="-DCMAKE_INSTALL_RPATH_USE_LINK_PATH=ON -DCMAKE_INSTALL_RPATH=$AOMP_INSTALL_DIR/lib:\$ORIGIN/../hsa/lib -DCMAKE_BUILD_TYPE=$BUILDTYPE -DATMI_HSA_INTEROP=on -DATMI_WITH_AOMP=on -DCMAKE_INSTALL_PREFIX=$INSTALL_ATMI -DROCM_DIR=$ROCM_DIR -DROCM_VERSION=$ROCM_VERSION -DLLVM_DIR=$LLVM_DIR"
