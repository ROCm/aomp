...
MYCMAKEOPTS="-DCMAKE_BUILD_TYPE=$BUILD_TYPE
 ...
 -DLLVM_EXTERNAL_PROJECTS=rocm-device-libs;rocm-compilersupport
 -DLLVM_EXTERNAL_ROCM_DEVICE_LIBS_SOURCE_DIR=$AOMP_REPOS/$AOMP_PROJECT_REPO_NAME/amd/device-libs
 -DLLVM_EXTERNAL_ROCM_COMPILERSUPPORT_SOURCE_DIR=$AOMP_REPOS/$AOMP_PROJECT_REPO_NAME/amd/comgr
 -DEXTERNAL_PROJECT_HSA_PATH=$AOMP_REPOS/$AOMP_ROCR_REPO_NAME/src
 -DLIBOMPTARGET_EXTERNAL_PROJECT_HSA_PATH=$AOMP_REPOS/$AOMP_ROCR_REPO_NAME/src
 -DLIBOMP_COPY_EXPORTS=OFF
 -DLIBOMPTARGET_ENABLE_DEBUG=ON
 -DLIBOMPTARGET_AMDGCN_GFXLIST=gfx90a
...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then
   ...
   ${AOMP_CMAKE} $MYCMAKEOPTS $AOMP_ORIGIN_RPATH $AOMP_REPOS/$AOMP_PROJECT_REPO_NAME/llvm 2>&1
   if [ $? != 0 ] ; then 
