...
if [ "$1" != "install" ] ; then 
   ...
   if [ $COPYSOURCE ] ; then 
      ...
      if [ $COPYSOURCE ] ; then 
         ...
         echo "cmake $MYCMAKEOPTS -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR $AOMP_REPOS/$AOMP_LIBDEVICE_REPO_NAME"
         cmake $MYCMAKEOPTS -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR $AOMP_REPOS/$AOMP_LIBDEVICE_REPO_NAME
      fi
      if [ $? != 0 ] ; then 
