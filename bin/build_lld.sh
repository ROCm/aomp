#!/bin/bash
# 
#  build_lld.sh:  Script to build the lld component of the AOMP compiler. 
#
#
...
INSTALL_LLD=${INSTALL_LLD:-$AOMP_INSTALL_DIR}
...
MYCMAKEOPTS="-DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_INSTALL_PREFIX=$INSTALL_LLD -DLLVM_DIR=$AOMP_INSTALL_DIR/lib/cmake/llvm -DLLVM_ENABLE_ASSERTIONS=ON -DLLVM_TARGETS_TO_BUILD=$TARGETS_TO_BUILD $COMPILERS -DLLVM_VERSION_SUFFIX=_AOMP_Version_$AOMP_VERSION_STRING -DCMAKE_INSTALL_RPATH_USE_LINK_PATH=ON -DCMAKE_INSTALL_RPATH=$AOMP_INSTALL_DIR/lib"
...
if [ "$1" == "install" ] ; then 
   $SUDO mkdir -p $INSTALL_LLD
   $SUDO touch $INSTALL_LLD/testfile
   if [ $? != 0 ] ; then 
      echo "ERROR: No update access to $INSTALL_LLD"
      exit 1
   fi
...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then
   echo 
   echo "This is a FRESH START. ERASING any previous builds in $BUILD_DIR/build/$AOMP_LLD_REPO_NAME"
   echo "Use ""$0 nocmake"" or ""$0 install"" to avoid FRESH START."
   rm -rf $BUILD_DIR/build/$AOMP_LLD_REPO_NAME
   mkdir -p $BUILD_DIR/build/$AOMP_LLD_REPO_NAME
   ...
   if [ ! -d $BUILD_DIR/build/$AOMP_LLD_REPO_NAME ] ; then 
      echo "ERROR: The build directory $BUILD_DIR/build/$AOMP_LLD_REPO_NAME does not exist"
...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then
   echo
   echo " -----Running cmake ---- " 
   echo cmake $MYCMAKEOPTS  $BUILD_DIR/$AOMP_LLD_REPO_NAME
   cmake $MYCMAKEOPTS  $BUILD_DIR/$AOMP_LLD_REPO_NAME 2>&1
   if [ $? != 0 ] ; then 
...
if [ "$1" == "install" ] ; then
   echo " -----Installing to $INSTALL_LLD ---- " 
   $SUDO make install 
   if [ $? != 0 ] ; then 
