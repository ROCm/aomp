...
if [ "$AOMP_BUILD_SANITIZER" == 1 ]; then
   ASAN_HOST_LIB_PATH=$(${AOMP_INSTALL_DIR}/bin/clang --print-runtime-dir)
   ASAN_HOST_LIB_RPATH="-Wl,-rpath=$ASAN_HOST_LIB_PATH -L$ASAN_HOST_LIB_PATH"
   ASAN_FLAGS="-fsanitize=address -shared-libasan -g $ASAN_HOST_LIB_RPATH -I$COMP_INC_DIR"
   ASAN_CMAKE_OPTS="$MYCMAKEOPTS -DCMAKE_PREFIX_PATH=$AOMP/lib/asan/cmake $AOMP_ASAN_ORIGIN_RPATH -DCMAKE_INSTALL_BINDIR=bin/asan -DCMAKE_INSTALL_LIBDIR=lib/asan"
fi
