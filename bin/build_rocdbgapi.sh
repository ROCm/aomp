...
cfgvar() {
  get_config_var_string rocdbgapi "$1"
}

cfgbool() {
  get_config_var_bool rocdbgapi "$1"
}

INSTALL_ROCDBGAPI=${INSTALL_ROCDBGAPI:-$AOMP_INSTALL_DIR}
REPO_DIR="$(cfgvar AOMP_REPOS)/$(cfgvar AOMP_DBGAPI_REPO_NAME)"

if [ "$1" == "-h" ] || [ "$1" == "help" ] || [ "$1" == "-help" ] ; then
  echo " "
  echo " This script builds the ROCM runtime libraries"
  echo " It gets the source from:  $REPO_DIR"
  echo " It builds libraries in:   $(cfgvar BUILD_DIR)/rocdbgapi"
...
task_precheck() {
   ...
   check_writable_installdir "$1" "$(cfgvar INSTALL_ROCDBGAPI)"
}
...
task_cmake() {
   ...
   InstallDir="$(cfgvar INSTALL_ROCDBGAPI)"
   ...
   MYCMAKEOPTS=(-DCMAKE_BUILD_TYPE=Release
                ...
                -DCMAKE_PREFIX_PATH="$AOMP_INSTALL_DIR;$InstallDir/include"
   ...
   echo " -----Running cmake for rocdbgapi $Cfg ---- "
   echo "$AompCmake $(shquot "${MYCMAKEOPTS[@]}") $SrcDir"

   if ! "$AompCmake" "${MYCMAKEOPTS[@]}" "$SrcDir"; then
      echo "ERROR rocdbgapi $Cfg cmake failed. cmake flags"
...
task_build() {
   ...
   pushd "$BuildDir" >& /dev/null || exit
   echo " -----Running make for rocdbgapi $Cfg ---- "
   echo "make -j $Jobs"
   if ! make -j "$Jobs"; then
      echo " "
