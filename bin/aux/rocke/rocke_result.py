...
# Status for a row that certified nothing: no device, no reference build, this
# architecture absent. The dashboard counts a status matching this separately from
# pass and fail, which is the only way a test that regresses into a skip is visible
# -- reported as 0 it reads as an unchanged pass against yesterday's baseline. A
# toolchain-caused skip is not this: it is a failure and carries a non-zero status.
STATUS_CHECK = "Check"
