...
from rocke_tiers import TIER_CAPABLE, TIER_COMPILER, TIER_LOGIC
...
def _install_native_probe() -> None:
    ...
    def probed(self, *args, **kwargs):
        # This wrapper sits on the hot path of every native call rocKE makes, so it
        # must never be the reason one fails. `_lib_resolver` is rocKE's private
        # attribute: if it is renamed, attribute the call generically rather than
        # raising an AttributeError that would surface as hundreds of rocKE-tier
        # failures caused by this harness.
        try:
            resolver = getattr(getattr(self, "_lib_resolver", None), "__module__", "") or ""
            family = "comgr" if resolver.endswith("comgr") else (
                "hip" if resolver.endswith("hip_module") else "native"
            )
            _record(f"call:{family}")
        except Exception:  # noqa: BLE001 - never break a call just to record evidence
            pass
...
def pytest_sessionfinish(session, exitstatus):  # noqa: ANN001, ARG001
    ...
    # Best-effort like the row log: this plugin is a guest in rocKE's session, and a
    # write failure here must cost the run its tiers, not its test results. The
    # converter reports the absent manifest as a red row of its own.
    try:
        tmp = f"{out}.tmp"
        ...
        print(f"rocke_relevance: cannot write {out}: {exc}", file=sys.stderr)
