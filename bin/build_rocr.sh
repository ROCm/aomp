...
if [ "$1" != "nocmake" ] && [ "$1" != "install" ] ; then 
   ...
   MYCMAKEOPTS="-DCMAKE_INSTALL_PREFIX=$INSTALL_ROCM -DCMAKE_BUILD_TYPE=$BUILDTYPE -DCMAKE_PREFIX_PATH=$ROCM_DIR -DIMAGE_SUPPORT=OFF $AOMP_ORIGIN_RPATH -DCMAKE_INSTALL_LIBDIR=lib"
   ...
   if [ "$AOMP_BUILD_SANITIZER" == 1 ] ; then
      ASAN_CMAKE_OPTS="-DCMAKE_C_COMPILER=${AOMP}/lib/llvm/bin/clang -DCMAKE_CXX_COMPILER=${AOMP}/lib/llvm/bin/clang++ -DCMAKE_INSTALL_PREFIX=$INSTALL_ROCM -DCMAKE_INSTALL_LIBDIR=$INSTALL_ROCM/lib/asan -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_PREFIX_PATH=$ROCM_DIR/lib/asan/cmake -DIMAGE_SUPPORT=OFF $AOMP_ASAN_ORIGIN_RPATH"
