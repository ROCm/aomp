...
if [ "$AOMP_PROC" == "ppc64le" ] ; then
    MYCMAKEOPTS="-DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_INSTALL_PREFIX=$INSTALL_FLANG -DLLVM_ENABLE_ASSERTIONS=ON -DCMAKE_Fortran_COMPILER=$AOMP_INSTALL_DIR/lib/llvm/bin/flang -DLLVM_TARGETS_TO_BUILD=$TARGETS_TO_BUILD "
else
    MYCMAKEOPTS="-DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_INSTALL_PREFIX=$INSTALL_FLANG -DLLVM_ENABLE_ASSERTIONS=ON -DLLVM_CONFIG=$INSTALL_FLANG/bin/llvm-config -DCMAKE_CXX_COMPILER=$AOMP_INSTALL_DIR/lib/llvm/bin/clang++ -DCMAKE_C_COMPILER=$AOMP_INSTALL_DIR/lib/llvm/bin/clang  -DLLVM_TARGETS_TO_BUILD=$TARGETS_TO_BUILD "
fi
