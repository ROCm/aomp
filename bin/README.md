...
| AOMP_VERSION_MOD      | 0                  | This implies the next release will be AOMP_18.0-0.
...
AOMP=$HOME/rocm/aomp_17.0-3.  Be careful that if you set AOMP for testing purposes that
...
For example, when building AOMP version 18.0-0 the install scripts will put all files and directories
in $HOME/rocm/aomp_18.0-0 and create a symbolic link as follows:

```
ln -sf ${AOMP}_18.0-0 ${AOMP}
```
...
For example, if the aomp symbolic link currently points to aomp_18.0-0 and you want to test aomp_17.0-3, do this:

```
export AOMP=$HOME/rocm/aomp_17.0-3
```
